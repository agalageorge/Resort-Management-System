...
/* =========================
   FORCE BUSH BAR DEPARTMENT
========================= */
$frontDeskSql = "SELECT department_id, name FROM departments WHERE name = 'Bush Bar' LIMIT 1";
$frontDeskResult = $conn->query($frontDeskSql);
if (!$frontDeskResult || $frontDeskResult->num_rows === 0) {
    die("Error: Bush Bar department not found.");
}
$frontDeskData   = $frontDeskResult->fetch_assoc();
$department_id   = (int)$frontDeskData['department_id'];
$department_name = $frontDeskData['name'];

// Define this variable (was missing → caused the warning)
$canViewAll = false;  // Change to true only if this page should allow viewing all departments
    ...
    <div class="main_content">
        ...
        <div class="user_contents">
            ...
            <div class="user_content_top">
                <nav><a href="bush_bar_payment_type_search.php"><i class="fa fa-cart-plus" aria-hidden="true"></i> Payment Method</a></nav>
                <nav><a href="bush_bar_item_search.php"><i class="fa fa-cart-plus" aria-hidden="true"></i> Item Name</a></nav>
            </div>
