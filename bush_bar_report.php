...
// Assume this is Bush Bar restricted view (change if some users can see all)
$canViewAll = false; // ← change to true if admin / superadmin

/* ================================
   GET BUSH BAR DEPARTMENT ID
================================ */
$deptSql = "SELECT department_id, name FROM departments WHERE name = 'Bush Bar' LIMIT 1";
$deptResult = $conn->query($deptSql);
if (!$deptResult) {
    die("Database error: " . $conn->error);
}
if ($deptResult->num_rows === 0) {
    die("Bush Bar department not found in database.");
}
